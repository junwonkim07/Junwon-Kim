...
export const useThemeToggle = ({
  ...
  const toggleTheme = useCallback(() => {
    // Branch on resolvedTheme, not theme. next-themes defaults `theme` to
    // "system", so `theme === "light"` was false on a first click no matter how
    // the page actually looked and every first toggle went to light. The icon
    // was also flipped optimistically here, so it disagreed with the theme
    // until the resolvedTheme effect corrected it — that correction is the
    // flicker. isDark is now derived from resolvedTheme alone.
    ...
    // Starting a transition while one is still running aborts the first, and
    // its promise rejects with InvalidStateError. Skip the wipe in that case
    // and just swap, then swallow the abort so it is not an unhandled rejection.
    ...
    // ready, updateCallbackDone and finished are all created up front. An
    // aborted transition rejects each one, and any without a handler surfaces
    // as an unhandled rejection, so all three are silenced.
