...
export function GithubCalendar({
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                // Upstream points at github-contributions-api.deno.dev, which
                // stopped existing when Deno Deploy Classic was sunset on
                // 2026-07-20 — every username returned DEPLOYMENT_NOT_FOUND.
                // This host is live but returns a flat day list with numeric
                // levels, so it is reshaped into the weeks-of-days grid and the
                // quartile names the component renders from.
                const response = await fetch(
                    `https://github-contributions-api.jogruber.de/v4/${username}?y=last`
                )
                if (!response.ok) {
                    throw new Error(`GitHub data unavailable (${response.status})`)
                }
                const raw: {
                ...
                const days: ContributionDay[] = raw.contributions.map((d) => ({
                    ...
                    contributionLevel: LEVELS[Math.min(d.level, 4)],
                    color: "",
                }))

                // Each column is one Sunday-to-Saturday week. The series rarely
                // starts on a Sunday or ends on a Saturday, so both ends are
                // padded with zero-count placeholders — dropping the padding
                // instead left the first and last columns short and the grid
                // looked half-empty.
                const blank = (): ContributionDay => ({
                    date: "",
                    contributionCount: 0,
                ...
                const lead = days.length ? new Date(days[0].date).getUTCDay() : 0
                ...
                setData({
                    contributions: weeks,
                    totalContributions:
                        raw.total?.lastYear ?? Object.values(raw.total ?? {})[0] ?? 0,
                })
            } catch (err) {
    ...
    return (
        <div className={cn("w-full flex flex-col gap-4", className)}>
            ...
            <div
                className="relative flex w-full flex-nowrap gap-[3px]"
                onMouseLeave={() => {
                    setHoveredDate(null)
            ...
            >
                ...
                {weeks.map((week, weekIndex) => (
                    // flex-1 instead of a fixed 14px column so the 53 weeks
                    // divide the container width and the grid spans it fully.
